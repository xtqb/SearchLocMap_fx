...
public class BDNumChangeReceiver extends BroadcastReceiver {
    ...
    public void onReceive(Context context, Intent intent) {
        ...
        //收到广播后去拿到北斗号
        String bdNum = BaseUtils.getDipperNum(context);
        LogUtil.e("切换后本机北斗号=="+bdNum);
        //本地北斗为空 不处理
        if (TextUtils.isEmpty(bdNum)) {
            ToastUtil.showToast("北斗卡号为空,请先安装北斗卡");
            return;
        }
        //本地北斗未改变  不处理
        ...
        //上传    判断有无网络
        if (BaseUtils.isNetConnected(context)) {
            //有网络  通知服务器北斗号
            ...
            if (!bdNum.equals(SpUtils.getString(Constants.BD_NUM_lOC_DEF, ""))) {
                ...
                intent1.setAction("com.lhzw.intent.action_UPLOAD_SERVICE");
